...
These identity changes should be applied in any cases where a process has the potential to make a change to the content, even if it isn't currently doing so. For example, a vision switcher's output should be identified with its own `Source` ID, even if it is currently acting as a pass-through for a single input.
...
*   **Sample Rate Conversion**
    ...
    *   Note: Includes video frame rate conversion which might involve interpolation but does not include time stretches such as 24Hz to 25Hz where the same content is played back without modification but at a different rate (ie. duration modified). For these cases see 'Slow Motion / Speed Up' and 'Frame Synchroniser / Rate Conforming'.
