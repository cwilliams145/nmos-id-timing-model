...
* All the `Flow Representation`s that implement a `Flow` are representations of (almost) the same quality / accuracy (in other words, they all: result in (almost) the same presentation; represent (almost) the same baseband signal).
